...
const getBuildStageAdjustment = ({
  ...
  if (buildStage === 2) {
    if (isNextCore) {
      return {
        points: coreCompletion >= 50 ? 24 : 40,
...
const getBuildQualityAdjustment = ({
  ...
  if (!item) {
    return { points: 0, reasons: [], contributions: [] };
  }
  ...
  (championBuild?.preferredStats || []).forEach((statName) => {
    if (ITEM_STAT_MATCHERS[statName]?.(item)) {
      const impact = `${itemName} supplies preferred ${statName.replaceAll("_", " ")}.`;
      points += 3;
      reasons.push(impact);
      contributions.push({ value: 3, impact });
    }
  });

  (championBuild?.avoidedStats || []).forEach((statName) => {
    if (ITEM_STAT_MATCHERS[statName]?.(item)) {
      const impact = `${itemName} spends gold on avoided ${statName.replaceAll("_", " ")}.`;
      points -= 6;
      reasons.push(impact);
      contributions.push({ value: -6, impact });
    }
  });
  ...
  if (
    ...
    const impact = `${itemName} fits the champion's preferred playstyle.`;
    points += 5;
    reasons.push(impact);
    contributions.push({ value: 5, impact });
  }

  return { points, reasons, contributions };
};
...
const getGameStateAdjustment = ({
  ...
  if (
    ...
    const impact = `${gameContext.phase} game timing keeps core progression valuable.`;
    reasons.push(impact);
    contributions.push({
      value: phaseRules.core_progression,
      impact,
    });
  }
  ...
  const addStrategyWeight = (rules, reason) => {
    ...
    if (value !== 0) {
      reasons.push(reason);
      contributions.push({ value, impact: reason });
    }
  };
  ...
  if (
    itemName === buildProgress.nextCoreItem &&
    gameContext.performance === "behind"
  ) {
    points -= 12;
    const impact =
      "Being behind reduces the value of forcing the default core order.";
    reasons.push(impact);
    contributions.push({ value: -12, impact });
  }

  if (itemName === buildProgress.nextCoreItem && gameContext.highDeaths) {
    points -= 8;
    const impact =
      "Repeated deaths make an adaptive defensive purchase more urgent.";
    reasons.push(impact);
    contributions.push({ value: -8, impact });
  }
...
const getFallbackContributionImpact = ({
  category,
  itemName,
  points,
}) => {
  const direction = points > 0 ? "increases" : "reduces";
  const impacts = {
    archetype: `${itemName} ${points > 0 ? "fits" : "conflicts with"} the champion archetype.`,
    buildStage: `Current build stage ${direction} the priority of ${itemName}.`,
    coreBaseline: `${itemName} is part of the configured champion build.`,
    synergy: `${itemName} has champion-specific synergy.`,
    buildQuality: `${itemName} ${points > 0 ? "matches" : "conflicts with"} preferred build qualities.`,
    counterStrategy: `${itemName} ${points > 0 ? "answers" : "does not fit"} the enemy team style.`,
    gameState: `Current game state ${direction} the value of ${itemName}.`,
    historyStability: `${itemName} remains consistent with the previous recommendation.`,
    enemyAnalysis: `Enemy analysis ${direction} the value of ${itemName}.`,
    redundancy: `${itemName} overlaps with an item already owned.`,
    phaseScaling: `Game phase scales the impact of enemy signals for ${itemName}.`,
  };

  return impacts[category] || `${category} ${direction} the item score.`;
};

const buildScoreExplanation = (contributions = []) => {
  const ranked = contributions
    .filter((contribution) => contribution.source !== "base")
    .sort((a, b) => Math.abs(b.value) - Math.abs(a.value));

  return {
    contributions,
    positiveFactors: ranked
      .filter((contribution) => contribution.value > 0)
      .slice(0, 4),
    negativeFactors: ranked
      .filter((contribution) => contribution.value < 0)
      .slice(0, 4),
  };
};
...
const scoreItem = ({
  ...
  const contributions = [
    {
      source: "base",
      value: 50,
      impact: "Every valid candidate starts from the same baseline score.",
    },
  ];
  ...
  const synergyConditionMet =
    ...
    (synergyRule.condition === "snowball" &&
      ownsSnowballComponent &&
      (gameContext.performance === "ahead" || gameContext.highKills));
  ...
  buildQualityAdjustment.contributions.forEach((contribution) => {
    add(contribution.value, contribution.impact, "buildQuality");
  });
  ...
  gameStateAdjustment.contributions.forEach((contribution) => {
    add(contribution.value, contribution.impact, "gameState");
  });
  ...
  if (championBuild?.archetype === "ad_diver") {
    ...
    if (itemName === "Black Cleaver" && tiers.armor === "low") {
      add(
        -18,
        "Enemy armor is still low, so Black Cleaver is not an urgent adaptation.",
      );
    }
  ...
  switch (itemName) {
    ...
    case "Rabadon's Deathcap": {
      ...
      if (tiers.engage === "extreme" || tiers.dive === "extreme") {
        add(
          -34,
          "Extreme engage pressure makes a pure damage purchase too risky.",
        );
      } else if (
        isTierAtLeast(tiers.engage, "high") &&
        isTierAtLeast(tiers.dive, "high")
      ) {
        add(
          -14,
          "Heavy engage and dive reduce the safety of a greedy damage purchase.",
        );
      }
    ...
    case "Banshee's Veil": {
      ...
      if (threatProfile.diveThreat) {
        add(
          -12,
          "Against committed dive, stasis is usually more reliable than a spell shield.",
        );
      }
  ...
  if (gameStateAdjustment.enemyAnalysisMultiplier !== 1) {
    ...
    const phaseScalingDelta = adjustedEnemyScore - originalEnemyScore;
    score += phaseScalingDelta;
    scoreBreakdown.enemyAnalysis = adjustedEnemyScore;
    if (phaseScalingDelta !== 0) {
      contributions.push({
        source: "phaseScaling",
        value: phaseScalingDelta,
        impact: `${gameContext.phase} game scaling changes the weight of enemy-specific signals.`,
      });
    }
  }
  const confidence = clamp(Math.round(score), 0, 100);
  const explanation = buildScoreExplanation(contributions);
  const reasons = explanation.positiveFactors
...
const calculateRecommendationConfidence = ({
  ...
  const scoreGapContribution = Math.round(Math.min(scoreGap, 25) * 1.2);
  ...
  const enemySignalStrength = [3, 7, 12, 16][strongestEnemyTier] || 3;
  const value = clamp(
    20 + scoreGapContribution + progressionCertainty + enemySignalStrength,
    0,
    100,
  );
  const band = getConfidenceBand(value);
  const tierNames = ["low", "medium", "high", "extreme"];
  const strongestTierName = tierNames[strongestEnemyTier] || "low";
  const factors = [
    {
      source: "confidenceBase",
      value: 20,
      impact: "Every recommendation starts with a base confidence level.",
    },
    {
      source: "scoreGap",
      value: scoreGapContribution,
      impact:
        scoreGap <= 2
          ? `The top candidates are nearly tied (${Math.round(scoreGap)} point gap).`
          : `The best item leads the next candidate by ${Math.round(scoreGap)} points.`,
    },
    {
      source: "buildProgression",
      value: progressionCertainty,
      impact:
        scored[0]?.item === buildProgress.nextCoreItem
          ? "The recommendation matches the next configured core item."
          : buildProgress.completedCorePercentage >= 75
            ? "Most core items are complete, so adaptive choices are more reliable."
            : "The recommendation departs from core order, reducing certainty.",
    },
    {
      source: "enemySignalStrength",
      value: enemySignalStrength,
      impact: `The strongest detected enemy signal is ${strongestTierName}.`,
    },
  ];
  ...
  return {
    ...
    enemySignalStrength,
    explanation: {
      summary: `${band} confidence (${value}%): ${
        scoreGap <= 2
          ? "the leading options are effectively tied."
          : `the recommendation leads by ${Math.round(scoreGap)} score points.`
      }`,
      factors,
    },
  };
    ...
    return {
      best: {
        ...
        explanation: {
          ...
          confidence: {
            summary:
              "No confidence is available because this champion-role build is unsupported.",
            factors: [],
          },
        },
      },
  ...
  return {
    ...
    alternatives: alternatives.map((item) => ({
      ...
      explanation: {
        ...
        confidence: {
          summary: `${getConfidenceBand(item.confidence)} alternative confidence (${item.confidence}% normalized candidate score).`,
          factors: [
            {
              source: "candidateScore",
              value: item.confidence,
              impact:
                "Alternative confidence uses the candidate's normalized score; the primary recommendation confidence also considers score gap and progression certainty.",
            },
          ],
        },
      },
    ...
    debug: {
      ...
      candidateScores: scored.map((item) => ({
        ...
        contributions: item.explanation?.contributions || [],
      })),
      enemyScores: enemyAnalysis.scores,
      enemyTiers: enemyAnalysis.tiers,
      teamStyle: enemyAnalysis.teamStyle,
...
const getBuildAdvice = getItemAdvice;
const getAPMidBuildAdvice = getItemAdvice;
